...
- 人物档案、大纲、章节正文及写作计划 JSON 的**权威副本**存 **Cloudflare R2** 对象（非本地磁盘）
- 数据库仅保存 **R2 对象键**（`storage_prefix`、`relative_path`、`text_storage_key`、`storage_key` 等）与 `status`、`wordCount` 等元数据索引，不得将长篇正文写入 BLOB/TEXT 字段
...
**登录拦截点：** 新建作品、快捷开写、作品列表、续写、向导 L0–L5、知识库、AI 模型设置——均需已登录；未登录访问上述能力时跳转登录页（登录成功后回跳原目标 URL，实现见 [design.md](./design.md)）。
...
- id: REQ-001-AC-004  
  ears: 当未登录用户尝试访问创作类功能（含新建、快捷开写、向导、规划、创作进度、偏好设置）时，摸鱼小说系统应拒绝服务并跳转登录页，且不得创建 `userId` 或 R2 作品对象前缀；注册/登录页本身应对访客可访问。

- id: REQ-001-AC-005  
  ears: 在用户已登录的前提下，当用户进入设置区域时，摸鱼小说系统应提供统一导航，至少包含「创作偏好」与「AI 模型」两个子页，且子页切换不得丢失未保存的表单状态（除非用户确认离开）。
...
### 需求 REQ-002 - 项目工作台与 R2 作品命名空间

**用户故事：** 作为作者，我希望在「我的作品」中查看全部项目状态并一键续写未完成作品，且每部作品在 R2 中有独立对象键前缀存放所有创作文件。
...
- id: REQ-002-AC-002  
  ears: 在至少存在一个 `status` 为 `writing` 的项目的前提下，当用户进入首页时，摸鱼小说系统应展示续写入口及章节写作进度信息（如「12/20 章」），且不得将 Phase 4 校验进度文案用于 `writing` 状态。

- id: REQ-002-AC-005  
  ears: 在至少存在一个 `status` 为 `planning` 且规划产物已就绪的项目的前提下，当用户进入首页或作品列表时，摸鱼小说系统应展示「继续确认规划」类入口（跳转 L4），且不得展示章节写作进度式续写文案。

- id: REQ-002-AC-003  
  ears: 当用户新建作品时，摸鱼小说系统应分配唯一 R2 对象键前缀并在数据库中持久化 `storage_prefix`，且不得将小说正文存入数据库正文列。
...
- id: REQ-002-AC-007  
  ears: 在用户已登录且存在 `status` 为 `draft` 的项目的前提下，当用户在作品列表或仪表盘点击该项目时，摸鱼小说系统应跳转至创作向导（L1–L3 未完成的最近一步），且不得重新创建作品或清空已保存的创作配置。

- id: REQ-002-AC-008  
  ears: 在用户已登录且存在 `status` 为 `planning` 且 `planning_ready` 为 false 的项目的前提下，当用户在作品列表或仪表盘点击该项目时，摸鱼小说系统应跳转至规划生成中页面（轮询 `planning_jobs`），且不得展示 L4 规划确认或章节写作进度。

- id: REQ-002-AC-009  
  ears: 在至少存在一个 `status` 为 `validating` 的项目的前提下，当用户进入首页或作品列表时，摸鱼小说系统应展示「继续校验」类入口及校验进度信息（如「校验中 8/20 章」）；用户点击后应跳转至 Phase 4 校验进度视图（可与 L5 路由共用），且不得展示「生成本章」等 Phase 3 写作控件。

**实现步骤：**

1. 新建作品时分配 R2 对象键前缀：`{userId}/{projectId}-{slug}/`（写入 `projects.storage_prefix`）
2. 数据库 `projects` 表保存 `id`、`userId`、`title`、`storage_prefix`、`status`、时间戳
3. 列表页按 5 态筛选；首页/仪表盘入口映射：
   - `draft` → 继续向导（REQ-002-AC-007）
   - `planning` + `planning_ready=false` → 规划生成中（REQ-002-AC-008、REQ-008-AC-007）
   - `planning` + `planning_ready=true` → 继续确认规划 / L4（REQ-002-AC-005）
   - `writing` → 续写 / L5 创作进度
   - `validating` → 校验进度（REQ-002-AC-009、REQ-011-AC-005）
   - `completed` → 完成页 / 阅读 / 编辑 / 导出
4. 所有 API 校验 `userId` 与 `project.userId` 一致；`projects.status` 使用上表枚举值

**R2 作品对象布局（Bucket 内对象键，不含 Bucket 名）：**

```text
{storage_prefix}00-人物档案.md
{storage_prefix}01-大纲.md
{storage_prefix}02-写作计划.json
{storage_prefix}第01章-章节标题.md
{storage_prefix}第02章-章节标题.md
…
```
...
- id: REQ-003-AC-011  
  ears: 当未登录用户进入营销首页（`/(marketing)`）时，摸鱼小说系统应展示产品价值说明、登录/注册 CTA，以及跳转注册/登录页的入口；且该页不得调用需认证的创作类 API。

- id: REQ-003-AC-002  
  ears: 在用户存在 `status` 为 `writing` 的项目的前提下，当用户选择续写时，摸鱼小说系统应跳转至 L5 创作进度页，且不得重新执行 Phase 1。

- id: REQ-003-AC-012  
  ears: 在用户存在 `status` 为 `validating` 的项目的前提下，当用户选择续写时，摸鱼小说系统应跳转至 Phase 4 校验进度视图（见 REQ-002-AC-009、REQ-011-AC-005），且不得展示 Phase 3 写作控件。
...
1. 检测 `writing` → 续写至 L5；`validating` → 续写至校验进度（REQ-003-AC-012）；`planning`（`planning_ready=true`）→ 继续确认规划（REQ-002-AC-005）；`planning`（生成中）→ 规划轮询页（REQ-002-AC-008）
...
- id: REQ-006-AC-002  
  ears: 当用户选择或输入标题时，摸鱼小说系统应将 `novelName` 绑定至 R2 作品对象前缀、大纲文件头及写作计划 JSON。
...
2. 用户选择其一或输入自定义标题
3. 持久化 `title` / `novelName`；若 `storage_prefix` 尚未分配，以标题参与 slug 生成
4. 进入 L4 规划阶段
...
**用户故事：** 作为平台与用户，我希望人物、大纲、章节均以 Markdown 存于 Cloudflare R2，数据库仅保留 R2 对象键索引。

#### 验收标准

- id: REQ-007-AC-001  
  ears: 当系统生成人物档案、大纲或章节正文时，摸鱼小说系统应将内容写入 R2 上 `{storage_prefix}{relativePath}` 对应对象，且数据库仅更新 R2 路径类元数据（`storage_prefix`、`relative_path`）。

- id: REQ-007-AC-002  
  ears: 当用户在线阅读或导出章节时，摸鱼小说系统应通过 `R2StorageDriver` 从 R2 读取对应 `.md` 对象，而非从数据库正文字段读取。

- id: REQ-007-AC-003  
  ears: 当用户在 M8 保存编辑或确认 AI 润色时，摸鱼小说系统应写回 R2 上该章 `.md` 对象，并更新 `content_files` 索引时间戳。

- id: REQ-007-AC-004  
  ears: 当作品详情接口返回章节列表时，摸鱼小说系统应包含完整 R2 对象键或等价 `filePath` 字段（`storage_prefix` + `relative_path`），且不得内嵌章节全文（专用预览/下载接口除外）。

- id: REQ-007-AC-005  
  ears: 当任意环境（开发、测试、生产）启动 Web 或 Worker 进程时，摸鱼小说系统应通过 `R2StorageDriver` 连接 Cloudflare R2，且进程启动时校验 `R2_*` 环境变量齐全；**不得**将 MD/JSON 正文写入本地文件系统。

- id: REQ-007-AC-006  
  ears: 当删除作品或知识库文档时，摸鱼小说系统应通过 `deletePrefix` 或按 `storage_key` 删除 R2 对象；失败时写入 `storage_delete_pending` 由 Worker 补偿重试。

**实现步骤：**

1. 写路径：生成/更新 → **先写 R2**（`R2StorageDriver.writeText` / `writeBytes`）→ 再更新 `content_files` 索引（`fileType`, `relative_path`, `chapterNumber`, `updatedAt`）
2. 读路径：完整 R2 对象键 = `storage_prefix` + `relative_path`；`readText` / `readBytes`；可选短时缓存
3. 删除作品：`deletePrefix(storage_prefix)` 删除 R2 前缀下全部对象；失败时 `storage_delete_pending` 补偿
4. 配置：`config/storage.ts` 读取并校验 `R2_*`；`createStorageDriver()` **仅**返回 `R2StorageDriver`
5. 对象键：不含 Bucket 名；Bucket 由 `R2_BUCKET` 配置
6. 例外：创作配置、偏好、知识库绑定等短 JSON 可存库；知识库全文（`.txt`）、导出二进制（PDF/EPUB 等）存 R2 并记录 `text_storage_key` / `storage_key`
...
- id: REQ-008-AC-001  
  ears: 当 L3 标题确认后 Phase 2 启动时，摸鱼小说系统应在 R2 上创建作品对象前缀，并按模版生成 `00-人物档案.md`、`01-大纲.md`、`02-写作计划.json` 对象。

- id: REQ-008-AC-002  
  ears: 当生成 `01-大纲.md` 时，摸鱼小说系统应为每一规划章节填充 7 列表格（章节、标题、核心事件、承接上章、章首引子类型、悬念钩子、出场人物、场景列表）。

- id: REQ-008-AC-003  
  ears: 当生成 `02-写作计划.json` 时，摸鱼小说系统应将各章 `status` 设为 `pending`，且 `filePath` 为 `第NN章-{title}.md`（R2 对象键相对路径），与命名约定一致。
...
- id: REQ-008-AC-007  
  ears: 在 Phase 2 规划任务进行中的前提下，当用户进入该作品的规划生成页时，摸鱼小说系统应展示规划生成进度（轮询 `planning_jobs`），允许用户离开页面；用户返回后应恢复轮询并自动跳转至 L4（当 `planning_ready=true`）或展示失败态（当任务失败）。

- id: REQ-008-AC-008  
  ears: 当规划任务失败时，摸鱼小说系统应在规划生成页展示明确错误提示（用户可见文案为英文），并提供重试入口，且不得将 `planning_ready` 设为 true。

**实现步骤：**

| 序号 | 步骤 | 产出 | 模版/指南 |
|------|------|------|-----------|
| 1 | 创建/绑定 R2 对象前缀 | `storage_prefix` | `phase2-planning.md` §1 |
...
- id: REQ-009-AC-003  
  ears: 当用户选择并行批次模式时，摸鱼小说系统应持久化 `writingMode: parallel`，并分批处理章节，同时保持大纲连贯性；并行模式仅适用于自动创作节奏（`creationPace: auto`）。
...
| 并行批次 | 可与 `writingMode: parallel` 组合 | 不支持；`manual` 隐含串行 |
...
- id: REQ-011-AC-005  
  ears: 在项目 `status` 为 `validating` 的前提下，当用户进入该作品的进度页时，摸鱼小说系统应展示 Phase 4 校验进度（如「校验中 8/20 章」）与各章校验状态，且不得展示「生成本章」等 Phase 3 控件；校验失败重写期间应展示对应章节的处理状态。

- id: REQ-011-AC-006  
  ears: 当项目由 `validating` 切换为 `completed` 时，摸鱼小说系统应跳转或引导用户至完成页（`projects/[projectId]/complete` 或等价路由），并展示 REQ-012-AC-002 要求的统计信息与阅读/编辑/导出入口。

**实现步骤：**

1. 读 `02-写作计划.json` 与各章 `.md` 统计字数
2. 不合格章：`status` → `failed` → 触发 M5 重写（≤3 轮）
3. 校验进行中：L5 共用路由展示 Phase 4 进度（REQ-011-AC-005）
4. 全部通过 → `completed` + 完成页（总章数、总字数、各章状态；REQ-011-AC-006、REQ-012-AC-002）
5. 项目状态：`校验中` ↔ `创作中`（失败重写）→ `已完成`
...
- id: REQ-012-AC-003  
  ears: 当用户选择 MD、TXT、PDF 或 EPUB 导出格式并确认元数据时，摸鱼小说系统应从 R2 上的 Markdown 源对象组装并生成可下载文件（导出产物亦存 R2）。
...
| 3 | 预览目录与字数 |
| 4 | 从 R2 读取 `00-人物档案.md`、`01-大纲.md`、各章 `.md` 合并或转码 |
| 5 | 生成文件并提供下载；记录 `export_records` 导出历史 |
...
- id: REQ-015-AC-001  
  ears: 当用户上传支持的文档（txt、md、docx、doc、pdf、epub）时，摸鱼小说系统应解析文本，并在用户知识库中列出该参考及处理状态。
...
- id: REQ-015-AC-006  
  ears: 当展示生成可追溯信息时，摸鱼小说系统应标明该章引用了哪些参考文档及片段。
...
- id: REQ-018-AC-006  
  ears: 当 Server Actions 或受保护的 Route Handler 因速率限制被拒绝时，摸鱼小说系统应在当前页面以非阻塞方式展示英文错误提示（映射 `errors.rateLimited` 或等价 `errorKey`），且不得静默失败。
...
**策略：** 项目不分期裁剪；下列需求均为当前版本验收范围，实施顺序见 [tasks.md](./tasks.md)。

| 模块 | 需求 ID | 验收摘要 |
|------|---------|----------|
| 账户与偏好 | REQ-001 | 登录、偏好静默更新与向导预填 |
| 作品工作台 | REQ-002 | 五态列表、草稿继续向导、规划生成中/继续规划、创作/校验续写、归属隔离 |
| 初始化与快捷开写 | REQ-003 | L0 三入口、访客营销首页、提取结果页二选一 |
| 向导 L1–L3 | REQ-004 ~ REQ-006 | 核心三问、深度定制、标题 |
| 文件真源 | REQ-007 | MD/JSON 存 **R2**；库仅存 R2 对象键索引 |
| 规划 | REQ-008 | 人物、大纲、写作计划、规划生成中轮询、L4 确认 |
| 写作模式 | REQ-009 | 串行/并行 + 自动/手动 |
| 逐章创作 | REQ-010 | 单章子流程、Worker 自动、手动按序 |
| 全书校验 | REQ-011 | Phase 4 校验进度 UI、完成页跳转、完成报告 |
| 阅读与导出 | REQ-012 | 阅读器 + MD/TXT/PDF/EPUB 导出 |
| 编辑与润色 | REQ-013 | M8 编辑、一致性、AI 润色 diff |
| 自定义模型 | REQ-014 | OpenAI 兼容配置与路由 |
| 知识库 | REQ-015 | 上传/网址、绑定、RAG 注入 |
| 章节重生 | REQ-016 | 单章大纲/正文重生 |
| 状态机 | REQ-017 | 项目五态 + 章节四态 |
| 非功能 | REQ-018 | 续写、隔离、限流（含 UI 提示）、合规提示 |

**端到端验收路径（示例）：** 访客营销首页 → 登录 → 完整向导或快捷开写 → 规划生成中（若异步）→ L4 确认（可选绑定知识库）→ 自动或手动创作 → Phase 4 校验进度 → 完成页 → 阅读 → 编辑/润色 → 四格式导出 → 单章重生。

**界面与访问验收补充（P0，v1.4.0）：** 草稿恢复向导（REQ-002-AC-007）、规划 Job 轮询（REQ-008-AC-007/008）、校验进度与完成页（REQ-011-AC-005/006）、访客营销首页（REQ-003-AC-011）、设置统一导航（REQ-001-AC-005）、限流 UI 反馈（REQ-018-AC-006）。视觉规范与英文全站 UI 仍见 [design.md](./design.md)（P2 渐进）。
...
| 下游 | `design.md`（v1.4.0）、`data.md`、`api.md`（v1.4.0）、`tasks.md`（v1.4.0） |
...
| 6 | P0 | REQ-001~003、008、011、018 | **界面与访问验收补全**：草稿/规划中/校验中入口、访客营销页、设置导航、限流 UI、完成页路由 |
...
| 全量交付 | **REQ-001~018 均为当前基线**；并行写作、四格式导出、知识库、章节重生等均纳入 [tasks.md](./tasks.md) 全量任务 |
...
| 1.0.0 | 2026-05-20 | **定稿** |
| 1.1.0 | 2026-05-23 | 全量交付策略；`storage_prefix` 术语统一；附录 C 改为全量范围 |
| 1.2.0 | 2026-05-23 | REQ-007 增加 local/R2 双驱动验收；R2 为完整实现非预留 |
| 1.3.0 | 2026-05-23 | **R2 唯一存储**；库仅存 R2 对象键；移除本地文件系统驱动 |
| 1.4.0 | 2026-05-23 | 补全 UI/访问 P0 验收：草稿恢复、规划轮询、校验进度、营销首页、设置导航、限流 UI、完成页 |
